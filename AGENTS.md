...
**As of tip:** `4911f37e0144e80d72b105f5308d09e7578872f7` — concurrent approval stress pack **Complete** (`tests/test_concurrent_approval_stress.py`); living store-scope matrix landed (`docs/STORE_SCOPED_RBAC_TEST_MATRIX.md`, `ops/mvp/store-scope-rbac-matrix.json`, `pytest -m store_scope` / `tests/test_store_scope_rbac_matrix.py`) — SEC-M1…M5/L2 FIXED; overall `✅ HARDENED`; offline remote-wipe + Web Push delivery **PARTIAL** (automated VAPID wipe-via-push evidence + staging checklist; Completes still **MISSING**); ADR-005 membership **Complete** (flag default OFF — Complete ≠ prod default ON; store-scoped RBAC Complete still **MISSING** — matrix spine landed, residual continuum + product ALLOW sign-off + staging soak open — see `docs/STORE_SCOPED_RBAC_COMPLETE_REMAINING.md`); overall RBAC readiness **PARTIAL** (approval hardening + **percentage approval limits** + **first-class `export`/`view_cost` actions Complete** (commerce/dashboard/ops/AI + admin/settings/catalog export gates + unified cost omit; intentional role-gated admin dumps retained) on key surfaces — owner lockout, grantor subset, permission deps, dangerous-permission warnings, concurrent approval stress pack Complete (`FOR UPDATE` + process locks), optional `min_percent` — not RBAC Complete; temp membership expires_at Complete + elevation/break-glass MVP Complete; living store-scope matrix landed ≠ store-scoped Complete); paid billing **PARTIAL** (ADR-002 Phase E mock soak ready: portal + Checkout Session + signed webhook lifecycle + entitlement gate-ON allowlist evidence — Complete still **MISSING** / **ops-blocked** on live Stripe keys + staging soak; `PAID_BILLING_ENTITLEMENT_GATE_ENABLED` default **OFF**; `engineering_mock_soak_ready=true`; `paid_billing_complete_ops_blocked=true`); operator go-live readiness pack `docs/GO_LIVE_READINESS_CHECKLIST.md` (Completes still **MISSING**); Offline Complete still **MISSING**. Cookie + membership-scope + entitlement-gate flags default remain OFF (ops enable cutover).
