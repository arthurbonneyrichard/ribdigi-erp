...
- Scoped audit list/export ``details`` party ledger balance **redacted** for `store_manager`
  (``customer_balance`` / ``supplier_balance_before`` / ``supplier_balance_after`` on
  ``GET /audit-logs`` + CSV; FX + CLE master keys already redacted; document amounts /
  ``invoice_number`` / ``store_id`` / reason remain; admin keeps party ledger fields in details).
