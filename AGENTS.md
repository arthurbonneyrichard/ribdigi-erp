...
- Scoped audit list/export ``details`` CLE master **redacted** for `store_manager`
  (``credit_limit`` / ``available`` / ``current_balance`` / ``projected_balance`` /
  ``additional_amount`` on ``GET /audit-logs`` + CSV; FX keys already redacted;
  ``invoice_number`` / reason / operational amounts / ``store_id`` remain; admin
  keeps CLE master fields in details).
