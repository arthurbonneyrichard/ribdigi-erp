...
**As of tip:** `eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee` — audit details department_id redact (`department_id` on scoped `/audit-logs` JSON+CSV; expense/recurring JSON already redacted; amounts / status / `store_id` remain); audit party ledger + CLE master + CLE currency + additional_amount + audit FX landed; concurrent approval stress pack **Complete**; living store-scope matrix landed — SEC-M1…M5/L2 FIXED; overall `✅ HARDENED`; offline remote-wipe + Web Push delivery **PARTIAL** (Completes still **MISSING**); ADR-005 membership **Complete** (flag default OFF; store-scoped RBAC Complete still **MISSING** — residual continuum + product ALLOW sign-off + staging soak open — see `docs/STORE_SCOPED_RBAC_COMPLETE_REMAINING.md`); overall RBAC readiness **PARTIAL** (approval hardening + % limits + export/`view_cost` Complete + expires_at + elevation + concurrent approval stress Complete — not RBAC Complete); paid billing **PARTIAL** (ops-blocked); operator go-live pack `docs/GO_LIVE_READINESS_CHECKLIST.md` (Completes still **MISSING**); Offline Complete still **MISSING**. Cookie + membership-scope + entitlement-gate flags default remain OFF (ops enable cutover).`customer_balance` / `supplier_balance_before` / `supplier_balance_after` on scoped `/audit-logs` JSON+CSV; FX + CLE master already redacted; amounts / `invoice_number` / `store_id` remain); audit CLE master + CLE currency + additional_amount + audit FX landed; concurrent approval stress pack **Complete**; living store-scope matrix landed — SEC-M1…M5/L2 FIXED; overall `✅ HARDENED`; offline remote-wipe + Web Push delivery **PARTIAL** (Completes still **MISSING**); ADR-005 membership **Complete** (flag default OFF; store-scoped RBAC Complete still **MISSING** — residual continuum + product ALLOW sign-off + staging soak open — see `docs/STORE_SCOPED_RBAC_COMPLETE_REMAINING.md`); overall RBAC readiness **PARTIAL** (approval hardening + % limits + export/`view_cost` Complete + expires_at + elevation + concurrent approval stress Complete — not RBAC Complete); paid billing **PARTIAL** (ops-blocked); operator go-live pack `docs/GO_LIVE_READINESS_CHECKLIST.md` (Completes still **MISSING**); Offline Complete still **MISSING**. Cookie + membership-scope + entitlement-gate flags default remain OFF (ops enable cutover).
...
- Scoped audit list/export ``details`` ``department_id`` **redacted** for `store_manager`
  (``department_id`` on ``GET /audit-logs`` + CSV; expense/recurring JSON already redacted;
  departments list GET + assign/clear already denied; amounts / status / ``store_id`` remain;
  admin keeps ``department_id`` in details).
