...
- **AI packaging:** Chat is fail-closed (`503`) until `AI_ENABLED` + approved provider + key. Rule-based insights work without an LLM. Use `GET /ai/status` and `GET /ai/queries` (tenant-scoped; hashed/redacted prompts). Do not claim full BR-21 chat complete — see `docs/AI_SECURITY_MVP.md`.
