...
- Scoped audit list/export ``details`` FX fields **redacted** for `store_manager`
  (`currency` / `exchange_rate` / `total_base` / `invoice_total_base` / `balance_due_base` /
  `fx_gain_loss` / `settlement_base`; amounts / invoice numbers / ``store_id`` remain;
  invoice/payment/aging/CLE surfaces already redacted; admin keeps FX in details).
