...
6. **User↔store membership** remains deferred (ADR-005). Do not invent parallel
   ...
   **list/get/create/unpost/upload/delete ``attachment_url`` storage key redacted**)
   ...
   override remains company/finance admin) + **sales invoice credit-override audit redacted** (``credit_limit_overridden`` / ``credit_override_reason`` / ``by`` / ``at``; balance/status remain) + **sales invoice emailed_to redacted** (``delivery.to`` on send; ``emailed_at`` remains), and recurring expense list/export/
...
**As of tip:** `ab87edfcc262a030313f73c72c123d8f03cfd62b` — `docs: align PR #303 As of tip to emailed_to redact honesty HEAD` (feat ancestry `30c7e4c8d0`).
...
`has_logo` + server-side HTML/PDF embeds remain); sales-invoice credit-override audit redact (`credit_limit_overridden` / `credit_override_reason` / `by` / `at`; balance/status remain); sales-invoice `emailed_to` redact (`delivery.to` on send; `emailed_at` remains). Each slice closes
