...
- Admin job triggers: `GET /api/v1/jobs` (company_admin+) and `POST /jobs/{name}/run` (**super_admin** / **platform_owner** only). Pass `X-Tenant-ID` as the JWT `tenant_id` UUID (slug mismatch → cross-tenant 403).
