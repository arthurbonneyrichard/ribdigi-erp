...
**As of tip:** `bd636f38cf0c18633d8d4f1b9d5b7575f36da4aa` — **SEC-M1 + M3 + M4 + L2 FIXED**; **SEC-M2/M5 Phase C** on tip (flag OFF default; flag ON nulls JSON tokens + `cookie_session`; SPA apiFetch/authSession) — still **OPEN/PARTIAL**, not FIXED (staging soak + Phase D remain). Continuum CLE `invoice_total` redact honesty remains **PARTIAL**.  
