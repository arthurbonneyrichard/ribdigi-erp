...
**As of tip:** `PENDING_SEC_L2` — **SEC-M1 + M3 + M4 + L2 FIXED**; **SEC-M2/M5 Phase A+B remainder** on tip (flag OFF; SPA pages use `apiFetch`; dual-mode helpers remain) — still **OPEN/PARTIAL**, not FIXED. Continuum CLE `invoice_total` redact honesty remains **PARTIAL**.  
