...
- [x] Granular module/menu/record RBAC complete.
  - Module+action RBAC enforced on routes; `GET /roles` catalog (includes default `record_scope`); user create/patch/deactivate + `record_scope` override; bulk user CSV import (`GET /users/import/template`, `POST /users/import` dry-run/commit) with Users UI; Shell menu filtered from `/me` permissions; record scope `own`|`all` enforced on expenses, sales invoices/quotations/orders/returns, and purchasing requests/orders/GRNs/invoices/returns (`department` reserved / treated as `all` until org units exist). Approvals intentionally bypass own-scope. Remaining: department/branch scopes and custom roles.
...
- [ ] Purchasing/PO/GRN/supplier workflow complete.
  - Partial: supplier CRUD, purchase requests (draft→pending→approved/rejected→converted) with store_manager/company_admin approve (no self-approve) and one-click convert to draft PO, PO draft→email supplier (SMTP/console) then status=sent + resend with `emailed_at`/`emailed_to`, partial/full received, GRN posts stock + supplier balance, purchase invoices (from GRN or manual) draft→approve with unpaid/partial/paid/overdue + payment allocation, purchase returns draft→post with stock out + AP/debit note journal, supplier invoice file upload/download/delete on invoices, suggest-only supplier-invoice OCR (`POST .../ocr-suggest` + draft `PATCH` for supplier # / date / notes). Remaining: multi-level PR matrix beyond single approver step; PO amendment tracking.
- [ ] Sales/invoice/payment/customer workflow complete.
  - Partial: customer CRUD, quotations (send emails customer via SMTP/console then status=sent + resend, accept/reject/convert), sales orders (confirm/invoice), invoices draft→post with optional `store_id` (posts deduct that store’s warehouse stock) + email posted/partial/paid invoices to customer (SMTP/console) with `emailed_at`/`emailed_to` + resend (status unchanged), payments, sales returns with restock/discard + AR/journal, line `variant_id` on quote/order/invoice/return, salesperson + store performance reports with CSV/PDF/Excel export.
