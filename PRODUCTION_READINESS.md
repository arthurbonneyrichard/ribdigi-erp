...
- [ ] Sales/invoice/payment/customer workflow complete.
  - Partial: customer CRUD, quotations (send emails customer via SMTP/console then status=sent + resend, accept/reject/convert), sales orders (confirm soft-reserves store warehouse stock; cancel releases through processing; fulfillment `confirmed→processing→shipped→delivered` with timestamps; invoice from any pre-invoiced fulfillment status; invoice post consumes reservation then hard stock-out; delivery date/address + store_id), invoices draft→post with optional `store_id` (posts deduct that store’s warehouse stock) + print A4/thermal (`GET /sales/invoices/{id}/print`) + email posted/partial/paid invoices to customer (SMTP/console) with `emailed_at`/`emailed_to` + resend (status unchanged), payments, sales returns with restock/discard + credit note number on post + `settlement_method=adjust|refund` (refund cash/bank when return exceeds open AR) + AR/journal, line `variant_id` on quote/order/invoice/return, salesperson + store performance reports with CSV/PDF/Excel export. Remaining: configurable invoice numbering series; Approved/Sent/Overdue status labels.
