...
- [ ] Cross-tenant isolation integration tests pass for every tenant-owned resource.
  - Partial: SQLite suite covers product list scoping, foreign sales invoice 404, mismatched `X-Tenant-ID` (products, credit aging, users), suspend/activate login block, plus matrix coverage for customers/suppliers, credit statement/limit, product image, expenses (+ attachment), purchase invoices (+ attachment), tax rates, store inventory, POS sessions/drawer/receipt, notifications, backups (+ download), accounting accounts/journals, audit logs, bank connections, report schedules, cheques, stock transfers, PO print, product labels, media-key tenant mismatch (unit + planted wrong key on owned expense/logo HTTP 403), and tenant-scoped AI insights (chat remains 503 until provider). Remaining: OCR-suggest / receipt-send / import path edge cases if any still lack foreign-ID coverage.
