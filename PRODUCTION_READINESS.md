...
- [ ] Purchasing/PO/GRN/supplier workflow complete.
  - Partial: purchase requests (`purchase_requests` / items; draft→pending→approved/rejected/cancelled→converted; no self-approve; convert→draft PO; Requests UI tab), multi-level PR approval matrix (`tenants.purchase_request_approval_matrix`, `purchase_request_approval_actions`, amount steps + role gates, no duplicate-step actor; default Store Manager → Company Admin; `GET/PATCH /purchasing/settings`), supplier profile (code/type/category/status/address/notes/`payment_terms_days`, contacts CRUD, deactivate, purchase history; Suppliers UI tab), PO draft→sent→partial/full received (+ cancel), optional PO email on send (SMTP/console + `sent_at`/`emailed_to`) and print text (`GET .../orders/{id}/print`), GRN posts stock + supplier balance with per-line accepted/rejected + reason, batch/expiry on GRN lines (`goods_receipt_items.batch_*`, stock into `product_batches`, required when product `tracks_batches`), partial-receive UI, purchase invoices (from GRN or manual) draft→approve with unpaid/partial/paid/overdue + payment allocation, purchase returns draft→post with stock out + AP/debit note journal, supplier invoice file upload/download/delete on invoices, suggest-only supplier-invoice OCR (`POST .../ocr-suggest` + draft `PATCH` for supplier # / date / notes). Remaining: PO amendment tracking.
