...
- [x] Granular module/menu/record RBAC complete.
  - Module+action RBAC enforced on routes; `GET /roles` catalog (system + tenant custom); custom role CRUD (`POST/PATCH/PUT/DELETE /roles`, `custom_roles` table, copy-from system base, no wildcard `*` on custom roles; assigned users refreshed on permission change); user create/patch/deactivate + `record_scope` override (system or custom slug); Shell menu filtered from `/me` permissions; Users UI create/delete custom roles; record scope `own`|`all` enforced on expenses, sales invoices/quotations/orders/returns, and purchasing requests/orders/GRNs/returns/invoices (list/get/mutate; PR approve/reject bypass own-scope like expenses; `department` reserved / treated as `all` until org units exist). Remaining: department/branch scopes (org units).
