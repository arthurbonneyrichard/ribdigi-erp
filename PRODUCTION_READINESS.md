...
- [x] Granular module/menu/record RBAC complete.
  - Module+action RBAC enforced on routes; `GET /roles` catalog; user create/patch/deactivate APIs with session revoke; Shell menu filtered from `/me` permissions. Remaining: record-level scope (`own`/`department`/`all`) and custom roles.
