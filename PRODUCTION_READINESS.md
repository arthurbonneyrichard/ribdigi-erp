...
- [ ] Sales/invoice/payment/customer workflow complete.
  - Partial: customer CRUD, quotations (send emails customer via SMTP/console then status=sent + resend, accept/reject/convert), sales orders (confirm soft-reserves store warehouse stock; cancel releases; invoice post consumes reservation then hard stock-out; delivery date/address + store_id), invoices draft→post with optional `store_id` (posts deduct that store’s warehouse stock) + email posted/partial/paid invoices to customer (SMTP/console) with `emailed_at`/`emailed_to` + resend (status unchanged), payments, sales returns with restock/discard + AR/journal, line `variant_id` on quote/order/invoice/return, salesperson + store performance reports with CSV/PDF/Excel export. Remaining SO statuses: processing/shipped/delivered.
