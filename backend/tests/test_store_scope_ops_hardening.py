...
async def test_store_manager_product_master_writes_denied(client, db_session):
    """Product create/patch/variants/barcode/image denied for store_manager; reads allowed."""
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    product = seed["p1"]
    ...
    variant = m.ProductVariant(
        ...
        company_id=cid,
        product_id=product.id,
        name="Master Deny Variant",
        sku="SKU-MGR-VAR-DENY",
        cost_price=1,
        selling_price=2,
        is_active=True,
    )
    ...
    denied_patch = await ac.patch(
        ...
        headers=headers,
        json={"name": "Hijacked Product", "selling_price": 999},
    )
    assert denied_patch.status_code == 403, denied_patch.text
    assert denied_patch.json()["detail"]["code"] == "STORE_SCOPE_DENIED"
    ...
    denied_variant_patch = await ac.patch(
        ...
        headers=headers,
        json={"name": "Hijacked Variant", "selling_price": 50},
    )
    ...
    denied_image = await ac.post(
        f"/api/v1/products/{product.id}/image",
        headers=headers,
        files={"file": ("x.png", png, "image/png")},
    )
    assert denied_image.status_code == 403, denied_image.text
    assert denied_image.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    listed = await ac.get("/api/v1/products", headers=headers)
    assert listed.status_code == 200, listed.text
    assert any(row["id"] == product.id for row in listed.json()["data"])

    got = await ac.get(f"/api/v1/products/{product.id}", headers=headers)
    assert got.status_code == 200, got.text
    assert got.json()["data"]["id"] == product.id

    variants = await ac.get(f"/api/v1/products/{product.id}/variants", headers=headers)
    assert variants.status_code == 200, variants.text
    assert any(row["id"] == variant.id for row in variants.json()["data"])
