...
@pytest.mark.asyncio
async def test_store_manager_ai_customer_credit_limit_redacted(client, db_session):
    """store_manager AI insights/assist/export omit party credit_limit; admin intact."""
    from datetime import timedelta
    ...
    assert row.get("credit_limit") is None
    assert float(row["monetary"]) == pytest.approx(80.0)
    ...
    assert assist.status_code == 200, assist.text
    adata = assist.json()["data"]
    assert adata["customer"].get("credit_limit") is None
    answer = adata.get("answer") or ""
    assert "7777" not in answer
    assert "credit limit" not in answer.lower()

    exported = await ac.get("/api/v1/ai/customers/insights/export", headers=headers)
    assert exported.status_code == 200, exported.text
    assert "AI Credit Buyer" in exported.text
    assert "7777" not in exported.text
    ...
    assert float(admin_row.get("credit_limit") or 0) == pytest.approx(7777.0)
    ...
    assert "7777" in (admin_assist.json()["data"].get("answer") or "")
...
async def test_store_manager_user_admin_writes_denied(client, db_session):
    ...
    assert admin_got.status_code == 200, admin_got.text
    admin_body = admin_got.json()["data"]
    assert admin_body.get("email") == target.email
...
@pytest.mark.asyncio
async def test_store_manager_credit_limit_exceeded_invoice_total_base_redacted(
    client, db_session
):
    """CREDIT_LIMIT_EXCEEDED 409 nulls invoice_total_base for store_manager.
    ...
    store = m.Store(
        tenant_id=tid,
        company_id=cid,
        name="CLE Invoice Total Base Store",
        code="CLE-ITB",
        manager_id=mgr.id,
        is_active=True,
    )
    ...
    # Master fields remain redacted (separate closed theme)
    assert mgr_detail.get("credit_limit") is None
    assert mgr_detail.get("available") is None
...
@pytest.mark.asyncio
async def test_store_manager_credit_limit_exceeded_invoice_total_redacted(
    client, db_session
):
    """CREDIT_LIMIT_EXCEEDED 409 nulls invoice_total for store_manager.
...
async def test_store_manager_sales_invoice_currency_redacted(client, db_session):
    ...
    assert mgr_data.get("status") == "posted"
    # exchange_rate redacted separately (see exchange_rate test)
...
@pytest.mark.asyncio
async def test_store_manager_sales_invoice_exchange_rate_redacted(client, db_session):
    """Sales invoice list/get/export/print nulls exchange_rate for store_manager.

    Exchange-rates GET already denied; sales-invoice currency already redacted;
    credit payment exchange_rate already redacted. Invoice JSON/CSV/print must
    not re-dump company FX rate-table identity. Totals/status/balance remain;
    admin keeps exchange_rate; print text embed may retain the rate.
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    assert admin_got.status_code == 200, admin_got.text
    admin_data = admin_got.json()["data"]
    assert float(admin_data.get("exchange_rate") or 0) == pytest.approx(1.25)
    assert float(admin_data.get("total_amount") or 0) == 80.0
    ...
    assert mgr_data.get("currency") is None
    assert float(mgr_data.get("total_amount") or 0) == 80.0
    assert mgr_data.get("status") == "posted"
    ...
    assert float(row.get("total_amount") or 0) == 80.0

    admin_csv = await ac.get("/api/v1/sales/invoices/export", headers=admin_company)
    assert admin_csv.status_code == 200, admin_csv.text
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(r for r in admin_rows if r.get("invoice_number") == "INV-IXR-1")
    assert float(admin_csv_row.get("exchange_rate") or 0) == pytest.approx(1.25)

    mgr_csv = await ac.get("/api/v1/sales/invoices/export", headers=headers)
    assert mgr_csv.status_code == 200, mgr_csv.text
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(r for r in mgr_rows if r.get("invoice_number") == "INV-IXR-1")
    ...
    assert mgr_print.status_code == 200, mgr_print.text
    print_data = mgr_print.json()["data"]
    assert print_data.get("invoice", {}).get("exchange_rate") is None
    assert float(print_data.get("invoice", {}).get("total_amount") or 0) == 80.0
    assert isinstance(print_data.get("text"), str)


@pytest.mark.asyncio
async def test_store_manager_sales_invoice_balance_due_base_redacted(client, db_session):
    """Sales invoice list/get/export/print nulls balance_due_base for store_manager.

    Exchange-rates GET already denied; sales-invoice currency + exchange_rate
    already redacted; credit-aging document balance_due_base already redacted.
    Invoice JSON/CSV/print must not re-dump FX-converted base (balance_due ×
    rate). Totals/status/balance_due remain; admin keeps balance_due_base.
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    assert admin_got.status_code == 200, admin_got.text
    admin_data = admin_got.json()["data"]
    assert float(admin_data.get("balance_due_base") or 0) == pytest.approx(100.0)
    ...
    assert admin_csv.status_code == 200, admin_csv.text
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(r for r in admin_rows if r.get("invoice_number") == "INV-IBDB-1")
    assert float(admin_csv_row.get("balance_due_base") or 0) == pytest.approx(100.0)

    mgr_csv = await ac.get("/api/v1/sales/invoices/export", headers=headers)
    assert mgr_csv.status_code == 200, mgr_csv.text
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(r for r in mgr_rows if r.get("invoice_number") == "INV-IBDB-1")
    ...
    assert mgr_print.status_code == 200, mgr_print.text
    print_data = mgr_print.json()["data"]
    assert print_data.get("invoice", {}).get("balance_due_base") is None
    assert float(print_data.get("invoice", {}).get("balance_due") or 0) == pytest.approx(
        80.0
    )
    assert isinstance(print_data.get("text"), str)
...
@pytest.mark.asyncio
async def test_store_manager_quotation_emailed_to_redacted(client, db_session):
    """Quotation list/get/export/print nulls emailed_to for store_manager.
    ...
    send_payload = dashboard_scope_svc.apply_quotation_manager_redacts(
        {
            "emailed_to": recipient,
            "emailed_at": sent_at.isoformat(),
            "delivery": {"to": recipient, "mode": "console"},
        },
        [store.id],
    )
...
@pytest.mark.asyncio
@pytest.mark.asyncio
async def test_store_manager_purchase_invoice_currency_redacted(client, db_session):
    """Purchase invoice list/get/export nulls currency for store_manager.
    ...
    admin_data = admin_got.json()["data"]
    assert admin_data.get("currency") == "USD"
    assert float(admin_data.get("total_amount") or 0) == 75.0
    ...
    row = next(r for r in listed.json()["data"] if r["id"] == inv.id)
    assert row.get("currency") is None
    assert float(row.get("total_amount") or 0) == 75.0

    admin_csv = await ac.get("/api/v1/purchasing/invoices/export", headers=admin_company)
    assert admin_csv.status_code == 200, admin_csv.text
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(r for r in admin_rows if r.get("invoice_number") == "PI-CURR-1")
    assert admin_csv_row.get("currency") == "USD"

    mgr_csv = await ac.get("/api/v1/purchasing/invoices/export", headers=headers)
    assert mgr_csv.status_code == 200, mgr_csv.text
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(r for r in mgr_rows if r.get("invoice_number") == "PI-CURR-1")
    assert mgr_csv_row.get("currency") in (None, "")
...
async def test_store_manager_journal_entry_attachment_url_redacted(client, db_session):
    ...
    db_session.add(store)
    await db_session.flush()
    storage_key = f"{tid}/journals/url-redact-dump.pdf"
    entry = await accounting_svc.post_journal_entry(
        db_session,
        tenant_id=tid,
        user_id=mgr.id,
        description="JE attachment url redact",
        reference="JE-ATTACH-URL-1",
        store_id=store.id,
        company_id=cid,
        lines=[
    ...
    entry.attachment_url = storage_key
    await db_session.commit()
    ...
    admin_got = await ac.get(
        f"/api/v1/accounting/journal-entries/{entry.id}", headers=admin_headers
    )
    assert admin_got.status_code == 200, admin_got.text
    assert admin_got.json()["data"].get("attachment_url") == storage_key
    assert admin_got.json()["data"].get("has_attachment") is True
    ...
    listed = await ac.get("/api/v1/accounting/journal-entries", headers=headers)
    assert listed.status_code == 200, listed.text
    row = next(r for r in listed.json()["data"] if r["id"] == entry.id)
    assert row.get("has_attachment") is True
    assert row.get("attachment_url") is None

    got = await ac.get(
        f"/api/v1/accounting/journal-entries/{entry.id}", headers=headers
    )
    assert got.status_code == 200, got.text
    assert got.json()["data"].get("has_attachment") is True
    assert got.json()["data"].get("attachment_url") is None
...
@pytest.mark.asyncio
async def test_store_manager_document_company_email_redacted(client, db_session):
    """Print/receipt JSON nulls company_email for store_manager.

    Company profile GET + /me already omit email; print/receipt JSON must not
    re-dump company_email after address/phone redacts. company_name + has_logo
    remain; admin JSON keeps company_email; server-side text embed may retain it.
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    customer = seed["party1"]
    co = seed["c1"]
    co.email = "legal-secret@alpha.example.com"
    co.name = "Alpha Trading Co"
    await db_session.commit()
    ...
    assert admin_receipt.status_code == 200, admin_receipt.text
    admin_data = admin_receipt.json()["data"]
    assert admin_data.get("company_email") == "legal-secret@alpha.example.com"

    mgr_receipt = await ac.get(f"/api/v1/pos/sales/{sale.id}/receipt", headers=headers)
    assert mgr_receipt.status_code == 200, mgr_receipt.text
    mgr_data = mgr_receipt.json()["data"]
    assert mgr_data.get("company_name") == "Alpha Trading Co"
    assert mgr_data.get("company_email") is None
    assert "has_logo" in mgr_data
    ...
    assert admin_print.status_code == 200, admin_print.text
    admin_print_data = admin_print.json()["data"]
    assert admin_print_data.get("company_email") == "legal-secret@alpha.example.com"
    ...
    mgr_print_data = mgr_print.json()["data"]
    assert mgr_print_data.get("company_name") == "Alpha Trading Co"
    assert mgr_print_data.get("company_email") is None
    assert "has_logo" in mgr_print_data


@pytest.mark.asyncio
async def test_store_manager_document_tax_registration_redacted(client, db_session):
    """Print/receipt JSON nulls tax_registration_number for store_manager.

    Company profile GET + /me + tax-filing already omit TIN; print/receipt JSON
    must not re-dump tax_registration_number after company_email redacts.
    company_name + has_logo remain; admin JSON keeps TIN; server-side text embed
    may retain it.
    """
    ac, seed = client
    tid = seed["t1"].id
    ...
    customer = seed["party1"]
    co = seed["c1"]
    tin = "TIN-PRINT-SECRET-999"
    co.tax_registration_number = tin
    co.name = "Alpha Trading Co"
    await db_session.commit()
    ...
    db_session.add_all([sale, invoice, quote])
    await db_session.flush()
    ret = m.SalesReturn(
        tenant_id=tid,
        company_id=cid,
        return_number="SR-TRS-1",
        credit_note_number="CN-TRS-1",
        customer_id=customer.id,
        sales_invoice_id=invoice.id,
        status="posted",
        reason="damaged",
    ...
    for path in (
        ...
        assert admin_print.status_code == 200, admin_print.text
        admin_print_data = admin_print.json()["data"]
        assert admin_print_data.get("tax_registration_number") == tin

        mgr_print = await ac.get(path, headers=headers, params={"format": "text"})
        assert mgr_print.status_code == 200, mgr_print.text
        mgr_print_data = mgr_print.json()["data"]
        assert mgr_print_data.get("company_name") == "Alpha Trading Co"
        assert mgr_print_data.get("tax_registration_number") is None
        assert "has_logo" in mgr_print_data
...
@pytest.mark.asyncio
async def test_store_manager_receipt_print_template_redacted(client, db_session):
    """POS receipt JSON nulls receipt_print_template/default_paper for store_manager.

    Document-settings PATCH/export/preview + GET /tenants/me already denied; receipt
    JSON must not re-dump company print-template settings. company_name + has_logo +
    resolved paper remain; admin JSON keeps template fields; server-side text/PDF embeds remain.
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    co = seed["c1"]
    co.name = "Alpha Trading Co"
    co.receipt_print_template = "thermal_58"
    await db_session.commit()

    store = m.Store(
        tenant_id=tid,
        company_id=cid,
        name="Print Template Receipt Store",
        code="PTR1",
        manager_id=mgr.id,
        is_active=True,
    )
    ...
    assert admin_receipt.status_code == 200, admin_receipt.text
    admin_data = admin_receipt.json()["data"]
    assert admin_data.get("receipt_print_template") == "thermal_58"
    assert admin_data.get("default_paper") == "58mm"
    assert admin_data.get("paper") == "58mm"

    mgr_receipt = await ac.get(f"/api/v1/pos/sales/{sale.id}/receipt", headers=headers)
    assert mgr_receipt.status_code == 200, mgr_receipt.text
    mgr_data = mgr_receipt.json()["data"]
    assert mgr_data.get("company_name") == "Alpha Trading Co"
    assert mgr_data.get("receipt_print_template") is None
    assert mgr_data.get("default_paper") is None
    # Resolved paper for this render remains (operational width, not settings catalog).
    assert mgr_data.get("paper") == "58mm"
    assert "has_logo" in mgr_data
    assert isinstance(mgr_data.get("text"), str)
...
@pytest.mark.asyncio
async def test_store_manager_receipt_currency_redacted(client, db_session):
    """POS receipt JSON nulls currency for store_manager.

    Company profile GET + /me + /workspace switcher already omit company
    currency. Receipt JSON must not re-dump company/tenant currency.
    company_name + has_logo + totals remain; admin JSON keeps currency;
    server-side text embed may retain the code.
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    sale = m.Transaction(
        ...
        company_id=cid,
        tx_type="pos_sale",
        reference="POS-CUR-REF",
        session_id=sess.id,
        subtotal=9,
        tax=0,
        total=9,
        status="completed",
        payload={"items": [{"name": "Item", "quantity": 1, "unit_price": 9}]},
    )
    db_session.add(sale)
    await db_session.commit()
    ...
    admin_data = admin_receipt.json()["data"]
    assert admin_data.get("currency") == "USD"
    assert float(admin_data.get("total") or 0) == pytest.approx(9.0)
    ...
    assert mgr_data.get("company_name") == "Alpha Trading Co"
    assert mgr_data.get("currency") is None
    assert float(mgr_data.get("total") or 0) == pytest.approx(9.0)
    assert "has_logo" in mgr_data
    # Text rendered before JSON redacts — server-side embed may retain currency.
    assert isinstance(mgr_data.get("text"), str)
...
@pytest.mark.asyncio
async def test_store_manager_email_settings_read_denied(client, db_session):
    """GET /settings/email + CSV export denied for store_manager; admin remains."""
    ac, seed = client
    headers = await auth_headers(ac, email="mgr@alpha.example.com", tenant_slug="alpha")
    admin_headers = await auth_headers(
        ac,
    ...
    denied = await ac.get("/api/v1/settings/email", headers=headers)
    assert denied.status_code == 403, denied.text
    assert denied.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    denied_export = await ac.get("/api/v1/settings/email/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    ok = await ac.get("/api/v1/settings/email", headers=admin_headers)
    assert ok.status_code == 200, ok.text
    ok_export = await ac.get("/api/v1/settings/email/export", headers=admin_headers)
    assert ok_export.status_code == 200, ok_export.text
...
async def test_store_manager_sms_settings_read_denied(client, db_session):
    ...
    denied = await ac.get("/api/v1/settings/sms", headers=headers)
    assert denied.status_code == 403, denied.text
    assert denied.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    denied_export = await ac.get("/api/v1/settings/sms/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    ok = await ac.get("/api/v1/settings/sms", headers=admin_headers)
    assert ok.status_code == 200, ok.text
    ok_export = await ac.get("/api/v1/settings/sms/export", headers=admin_headers)
    assert ok_export.status_code == 200, ok_export.text
...
async def test_store_manager_api_keys_read_denied(client, db_session):
    ...
    denied_export = await ac.get("/api/v1/api-keys/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"
...
async def test_store_manager_backup_settings_read_denied(client, db_session):
    ...
    denied = await ac.get("/api/v1/backup/settings", headers=headers)
    assert denied.status_code == 403, denied.text
    assert denied.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    denied_export = await ac.get("/api/v1/backup/settings/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"
...
async def test_store_manager_backup_jobs_read_denied(client, db_session, tmp_path, monkeypatch):
    ...
    denied_export = await ac.get("/api/v1/backup/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"
...
async def test_store_manager_tenant_sessions_read_denied(client, db_session):
    ...
    denied = await ac.get("/api/v1/auth/tenant-sessions", headers=headers)
    assert denied.status_code == 403, denied.text
    assert denied.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    denied_export = await ac.get("/api/v1/auth/tenant-sessions/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    ok = await ac.get("/api/v1/auth/tenant-sessions", headers=admin_headers)
    assert ok.status_code == 200, ok.text
    ok_export = await ac.get("/api/v1/auth/tenant-sessions/export", headers=admin_headers)
    assert ok_export.status_code == 200, ok_export.text
...
async def test_store_manager_jobs_catalog_read_denied(client, db_session):
    ...
    denied = await ac.get("/api/v1/jobs", headers=headers)
    assert denied.status_code == 403, denied.text
    assert denied.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    denied_export = await ac.get("/api/v1/jobs/export", headers=headers)
    assert denied_export.status_code == 403, denied_export.text
    assert denied_export.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    ok = await ac.get("/api/v1/jobs", headers=admin_headers)
    assert ok.status_code == 200, ok.text
    ok_export = await ac.get("/api/v1/jobs/export", headers=admin_headers)
    assert ok_export.status_code == 200, ok_export.text
...
@pytest.mark.asyncio
async def test_store_manager_fiscal_period_reopen_denied(client, db_session):
    """POST /accounting/fiscal-period/reopen denied for store_manager; admin remains."""
    ac, seed = client
    headers = await auth_headers(ac, email="mgr@alpha.example.com", tenant_slug="alpha")
    admin_headers = await auth_headers(
        ac,
    ...
    denied = await ac.post("/api/v1/accounting/fiscal-period/reopen", headers=headers)
    assert denied.status_code == 403, denied.text
    assert denied.json()["detail"]["code"] == "STORE_SCOPE_DENIED"

    # Admin reopen is idempotent when period is already open.
...
@pytest.mark.asyncio
async def test_store_manager_sales_salesperson_email_redacted(client, db_session):
    """Sales-by-salesperson nulls staff email for store_manager.
    ...
    assert float(admin_row.get("revenue") or 0) >= 40.0
    ...
    assert float(mgr_data["total_revenue"]) == pytest.approx(40.0)
    ...
    assert admin_csv_row.get("email") == "admin@alpha.example.com"
...
@pytest.mark.asyncio
async def test_store_manager_sales_salesperson_role_redacted(client, db_session):
    """Sales-by-salesperson nulls staff role for store_manager.

    Users list/get already denied; salesperson email + full_name already
    redacted. Report JSON/CSV must not re-dump org roster RBAC role labels.
    Revenue / sale_count / user_id remain; admin JSON/CSV keep role.
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    admin_people = admin_sp.json()["data"]["salespeople"]
    admin_row = next(r for r in admin_people if r.get("user_id") == admin.id)
    assert admin_row.get("role") == "company_admin"
    assert admin_row.get("full_name") == "Alpha Admin"
    assert admin_row.get("email") == "admin@alpha.example.com"
    assert float(admin_row.get("revenue") or 0) >= 65.0

    mgr_sp = await ac.get("/api/v1/reports/sales/salesperson", headers=headers)
    assert mgr_sp.status_code == 200, mgr_sp.text
    mgr_data = mgr_sp.json()["data"]
    assert float(mgr_data["total_revenue"]) == pytest.approx(65.0)
    mgr_people = mgr_data["salespeople"]
    mgr_row = next(r for r in mgr_people if r.get("user_id") == admin.id)
    assert mgr_row.get("role") is None
    assert mgr_row.get("full_name") is None
    assert mgr_row.get("email") is None
    assert mgr_row.get("user_id") == admin.id
    assert float(mgr_row.get("revenue") or 0) == pytest.approx(65.0)
    ...
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(r for r in admin_rows if r.get("user_id") == admin.id)
    assert admin_csv_row.get("role") == "company_admin"
    ...
    assert mgr_csv.status_code == 200, mgr_csv.text
    assert "company_admin" not in mgr_csv.text
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(r for r in mgr_rows if r.get("user_id") == admin.id)
    assert mgr_csv_row.get("role") in (None, "")
    assert mgr_csv_row.get("full_name") in (None, "")
    assert mgr_csv_row.get("email") in (None, "")
...
@pytest.mark.asyncio
async def test_store_manager_credit_aging_document_currency_redacted(
    client, db_session
):
    """AR/AP aging nulls document currency for store_manager.
    ...
    assert admin_ar_doc.get("currency") == "USD"
    ...
    assert admin_ap_doc.get("currency") == "EUR"
    ...
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(
        r for r in admin_rows if r.get("document_number") == "INV-AGE-FX-1"
    )
    assert admin_csv_row.get("currency") == "USD"
    ...
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(
        r for r in mgr_rows if r.get("document_number") == "INV-AGE-FX-1"
    )
    assert mgr_csv_row.get("currency") in (None, "")
...
@pytest.mark.asyncio
async def test_store_manager_credit_aging_document_balance_due_base_redacted(
    client, db_session
):
    """AR/AP aging nulls document balance_due_base for store_manager.

    Exchange-rates GET already denied; aging document currency + exchange_rate
    already redacted; sales/purchase-invoice + credit-payment FX fields already
    redacted. Aging JSON/CSV must not re-dump FX-converted base amounts that
    imply the company rate table. balance_due / buckets / party name remain;
    admin keeps balance_due_base.
    """
    from datetime import timedelta
    ...
    admin_ar_doc = next(
        d
        for d in admin_ar.json()["data"]["documents"]
        if d.get("document_number") == "INV-AGE-BDB-1"
    )
    assert float(admin_ar_doc.get("balance_due_base") or 0) == pytest.approx(687.5)
    assert float(admin_ar_doc.get("balance_due") or 0) == pytest.approx(55.0)
    assert float(admin_ar_doc.get("exchange_rate") or 0) == pytest.approx(12.5)
    ...
    admin_ap_doc = next(
        d
        for d in admin_ap.json()["data"]["documents"]
        if d.get("document_number") == "PI-AGE-BDB-1"
    )
    assert float(admin_ap_doc.get("balance_due_base") or 0) == pytest.approx(363.0)
    ...
    assert mgr_ap_doc.get("balance_due_base") is None
    assert float(mgr_ap_doc.get("balance_due") or 0) == pytest.approx(33.0)
    ...
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(
        r for r in admin_rows if r.get("document_number") == "INV-AGE-BDB-1"
    )
    assert float(admin_csv_row.get("balance_due_base") or 0) == pytest.approx(687.5)
    ...
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(
        r for r in mgr_rows if r.get("document_number") == "INV-AGE-BDB-1"
    )
    assert mgr_csv_row.get("balance_due_base") in (None, "")
    assert float(mgr_csv_row.get("balance_due") or 0) == pytest.approx(55.0)
    assert mgr_csv_row.get("exchange_rate") in (None, "")
...
@pytest.mark.asyncio
async def test_store_manager_credit_payment_currency_redacted(client, db_session):
    """Customer/supplier payment list/export nulls currency for store_manager.
    ...
    assert admin_cust.status_code == 200, admin_cust.text
    admin_crow = next(
        r for r in admin_cust.json()["data"] if r.get("payment_number") == "CPAY-CUR-1"
    )
    assert admin_crow.get("currency") == "USD"
    ...
    mgr_crow = next(
        r for r in mgr_cust.json()["data"] if r.get("payment_number") == "CPAY-CUR-1"
    )
    ...
    assert admin_sup.status_code == 200, admin_sup.text
    admin_srow = next(
        r for r in admin_sup.json()["data"] if r.get("payment_number") == "SPAY-CUR-1"
    )
    assert admin_srow.get("currency") == "EUR"

    mgr_sup = await ac.get("/api/v1/credit/supplier-payments", headers=headers)
    assert mgr_sup.status_code == 200, mgr_sup.text
    mgr_srow = next(
        r for r in mgr_sup.json()["data"] if r.get("payment_number") == "SPAY-CUR-1"
    )
    ...
    assert admin_csv.status_code == 200, admin_csv.text
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(r for r in admin_rows if r.get("payment_number") == "CPAY-CUR-1")
    assert admin_csv_row.get("currency") == "USD"

    mgr_csv = await ac.get("/api/v1/credit/customer-payments/export", headers=headers)
    assert mgr_csv.status_code == 200, mgr_csv.text
    mgr_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(r for r in mgr_rows if r.get("payment_number") == "CPAY-CUR-1")
    assert mgr_csv_row.get("currency") in (None, "")
    ...
    admin_scsv_row = next(
        r for r in admin_srows if r.get("payment_number") == "SPAY-CUR-1"
    )
    assert admin_scsv_row.get("currency") == "EUR"
    ...
    mgr_scsv_row = next(r for r in mgr_srows if r.get("payment_number") == "SPAY-CUR-1")
    assert mgr_scsv_row.get("currency") in (None, "")
    assert float(mgr_scsv_row.get("amount") or 0) == pytest.approx(5.0)


@pytest.mark.asyncio
async def test_store_manager_credit_payment_fx_gain_loss_redacted(client, db_session):
    """Customer/supplier payment list/export nulls fx_gain_loss for store_manager.
    ...
    assert admin_cust.status_code == 200, admin_cust.text
    admin_crow = next(
        r for r in admin_cust.json()["data"] if r.get("payment_number") == "CPAY-FXGL-1"
    )
    assert float(admin_crow.get("fx_gain_loss") or 0) == pytest.approx(2.5)
    assert float(admin_crow.get("amount") or 0) == pytest.approx(10.0)

    mgr_cust = await ac.get("/api/v1/credit/customer-payments", headers=headers)
    assert mgr_cust.status_code == 200, mgr_cust.text
    mgr_crow = next(
        r for r in mgr_cust.json()["data"] if r.get("payment_number") == "CPAY-FXGL-1"
    )
    ...
    assert admin_sup.status_code == 200, admin_sup.text
    admin_srow = next(
        r for r in admin_sup.json()["data"] if r.get("payment_number") == "SPAY-FXGL-1"
    )
    assert float(admin_srow.get("fx_gain_loss") or 0) == pytest.approx(-1.1)

    mgr_sup = await ac.get("/api/v1/credit/supplier-payments", headers=headers)
    assert mgr_sup.status_code == 200, mgr_sup.text
    mgr_srow = next(
        r for r in mgr_sup.json()["data"] if r.get("payment_number") == "SPAY-FXGL-1"
    )
    ...
    admin_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(
        r for r in admin_rows if r.get("payment_number") == "CPAY-FXGL-1"
    )
    assert float(admin_csv_row.get("fx_gain_loss") or 0) == pytest.approx(2.5)
    ...
    admin_srows = list(csv.DictReader(io.StringIO(admin_scsv.text)))
    admin_scsv_row = next(
        r for r in admin_srows if r.get("payment_number") == "SPAY-FXGL-1"
    )
    assert float(admin_scsv_row.get("fx_gain_loss") or 0) == pytest.approx(-1.1)
...
@pytest.mark.asyncio
async def test_store_manager_audit_details_cle_master_redacted(client, db_session):
    """Audit list/export nulls CLE master fields inside details for store_manager.

    CREDIT_LIMIT_EXCEEDED 409 already redacts credit_limit / available /
    current_balance / projected_balance / additional_amount. Scoped audit
    JSON/CSV must not re-dump company credit master / base projection via
    credit_limit_override details. Operational amount / invoice_number /
    store_id / reason remain; admin keeps CLE master fields. FX keys already
    redacted separately; integrity hashes unchanged (redact on read only).
    """
    ac, seed = client
    tid = seed["t1"].id
    cid = seed["c1"].id
    ...
    cle_ev = await audit_svc.record_event(
        ...
        entity_id="inv-acm-1",
        details={
            "invoice_number": "INV-ACM-1",
            "customer_id": "cust-acm-1",
            "customer_name": "ACM Customer",
            "reason": "finance override for known good customer",
            "credit_limit": 100.0,
            "available": 20.0,
            "current_balance": 80.0,
            "projected_balance": 180.0,
    ...
    assert admin_listed.status_code == 200, admin_listed.text
    admin_row = next(r for r in admin_listed.json()["data"] if r["id"] == cle_ev.id)
    admin_details = admin_row.get("details") or {}
    assert float(admin_details.get("credit_limit") or 0) == pytest.approx(100.0)
    assert float(admin_details.get("available") or 0) == pytest.approx(20.0)
    assert float(admin_details.get("current_balance") or 0) == pytest.approx(80.0)
    assert float(admin_details.get("projected_balance") or 0) == pytest.approx(180.0)
    assert float(admin_details.get("additional_amount") or 0) == pytest.approx(100.0)
    assert admin_details.get("currency") == "USD"
    assert float(admin_details.get("invoice_total_base") or 0) == pytest.approx(100.0)

    mgr_listed = await ac.get(
        "/api/v1/audit-logs", headers=headers, params={"limit": 500}
    )
    assert mgr_listed.status_code == 200, mgr_listed.text
    mgr_row = next(r for r in mgr_listed.json()["data"] if r["id"] == cle_ev.id)
    mgr_details = mgr_row.get("details") or {}
    assert mgr_details.get("credit_limit") is None
    assert mgr_details.get("available") is None
    assert mgr_details.get("current_balance") is None
    assert mgr_details.get("projected_balance") is None
    ...
    assert admin_csv.status_code == 200, admin_csv.text
    admin_detail_rows = list(csv.DictReader(io.StringIO(admin_csv.text)))
    admin_csv_row = next(r for r in admin_detail_rows if "INV-ACM-1" in (r.get("details") or ""))
    admin_csv_details = json.loads(admin_csv_row["details"])
    assert float(admin_csv_details.get("credit_limit") or 0) == pytest.approx(100.0)
    assert float(admin_csv_details.get("additional_amount") or 0) == pytest.approx(100.0)
    ...
    assert mgr_csv.status_code == 200, mgr_csv.text
    mgr_detail_rows = list(csv.DictReader(io.StringIO(mgr_csv.text)))
    mgr_csv_row = next(r for r in mgr_detail_rows if "INV-ACM-1" in (r.get("details") or ""))
    mgr_csv_details = json.loads(mgr_csv_row["details"])
    assert mgr_csv_details.get("credit_limit") is None
    assert mgr_csv_details.get("available") is None
    assert mgr_csv_details.get("current_balance") is None
