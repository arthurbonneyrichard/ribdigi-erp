"""Purchases summary/suppliers from_date/to_date Query OpenAPI honesty (BR-14.3)."""
...
def test_iso_date_query_schema_for_purchases_summary_suppliers():
...
def test_purchases_summary_suppliers_date_ui_and_docs():
    ...
    assert "/reports/purchases/summary" in reports or "purchases/summary" in reports
    assert "/reports/purchases/suppliers" in reports or "purchases/suppliers" in reports
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "Purchases summary/suppliers date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "purchases/summary" in docs
    assert "purchases/suppliers" in docs


@pytest.mark.asyncio
async def test_purchases_summary_suppliers_date_query_blank_invalid_422(client):
    ac, _seed = client
