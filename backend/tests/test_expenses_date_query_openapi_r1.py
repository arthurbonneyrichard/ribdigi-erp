"""Expenses report from_date/to_date Query OpenAPI honesty (BR-14.4)."""
...
def test_iso_date_query_schema_for_expenses():
...
def test_expenses_date_ui_and_docs():
    ...
    assert "Expenses date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "expenses/summary" in docs
    assert "budget-vs-actual" in docs


@pytest.mark.asyncio
async def test_expenses_date_query_blank_invalid_422(client):
    ac, _seed = client
