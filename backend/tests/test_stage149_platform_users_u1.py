"""Stage 149 U1 — platform staff users CSV export."""
...
async def _platform_headers(ac, db_engine, email="staff-149u@ribdigi.example.com"):
...
@pytest.mark.asyncio
async def test_platform_users_export_csv(client, db_engine):
    ac, _seed = client
    headers = await _platform_headers(ac, db_engine)
    exported = await ac.get("/api/v1/platform/users/export", headers=headers)
    assert exported.status_code == 200, exported.text
    assert "text/csv" in exported.headers.get("content-type", "")
    text = exported.text
    header = text.splitlines()[0]
    assert "email" in header and "role" in header and "totp_enabled" in header
    assert "staff-149u@ribdigi.example.com" in text
    assert "password_hash" not in text
    assert "totp_secret" not in text


def test_platform_users_export_ui_u1():
    page = (ROOT / "frontend/app/platform/users/page.tsx").read_text(encoding="utf-8")
    assert "Stage 149" in page
    assert "/platform/users/export" in page
