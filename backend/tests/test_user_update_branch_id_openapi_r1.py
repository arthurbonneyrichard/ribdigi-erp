"""UserUpdate.branch_id ∈ UuidIdValue OpenAPI honesty (BR-3)."""
...
def test_user_update_branch_id_schema():
    assert _uuid.validate_python(f"  {_VALID}  ") == _VALID.lower()
    ok = UserUpdate.model_validate({"branch_id": f"  {_VALID}  "})
    assert ok.branch_id == _VALID.lower()
    omit_ok = UserUpdate.model_validate({"full_name": "Renamed"})
    assert omit_ok.branch_id is None
    nullish = UserUpdate.model_validate({"branch_id": None})
    assert nullish.branch_id is None
    for bad in ("", " ", "!!!", "http://evil", "not-a-uuid", "br_001", "a b"):
        with pytest.raises(ValidationError):
            UserUpdate.model_validate({"branch_id": bad})
...
def test_user_update_branch_id_ui_and_docs():
    ...
    assert "User update branch_id OpenAPI" in agents
    assert "UuidIdValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "Edit user branch" in docs
    assert "PATCH /users/{user_id}" in docs


@pytest.mark.asyncio
async def test_user_update_branch_id_api_blank_invalid_422(client, seeded):
    ac, seed = client
    ...
    created = await ac.post(
        ...
        headers=admin,
        json={
            "email": f"tip350-{suffix}@example.com",
            "full_name": "Tip 350 User",
