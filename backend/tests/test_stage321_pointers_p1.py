"""Stage 321 P1 — live DR pack RG pointers packaging."""
...
REGISTER = ROOT / "ops" / "mvp" / "live-dr-pack-rg-pointers.json"


def test_live_dr_pack_rg_pointers_register_p1():
    data = json.loads(REGISTER.read_text(encoding="utf-8"))
    assert data["stage"] == 321 and data["pack"] == "P1"
    ...
    assert any(
        s["id"] == "ldprp-dr-remaining" and s["status"] == "remaining" for s in data["steps"]
    )
    for rel in data["related"].values():
        assert (ROOT / rel).is_file(), rel


def test_live_dr_pack_rg_pointers_doc_p1():
    doc = (ROOT / "docs/LIVE_DR_PACK_RG_POINTERS_MVP.md").read_text(encoding="utf-8")
