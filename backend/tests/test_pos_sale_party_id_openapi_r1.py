"""PosSaleCreate.party_id ∈ UuidIdValue OpenAPI honesty (BR-8.1)."""
...
def test_pos_sale_party_id_schema():
    assert _uuid.validate_python(f"  {_VALID}  ") == _VALID.lower()
    omit = PosSaleCreate.model_validate({"items": _ITEMS})
    assert omit.party_id is None
    ok = PosSaleCreate.model_validate({"party_id": f"  {_VALID}  ", "items": _ITEMS})
    assert ok.party_id == _VALID.lower()
    for bad in ("", " ", "!!!", "http://evil", "not-a-uuid", "cust_001"):
        with pytest.raises(ValidationError):
            PosSaleCreate.model_validate({"party_id": bad, "items": _ITEMS})


def test_pos_sale_party_id_ui_and_docs():
    page = (ROOT / "frontend/app/pos/page.tsx").read_text(encoding="utf-8")
    assert 'aria-label="POS customer"' in page
    assert "party_id: customerId.trim() || null" in page
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "POS sale party_id OpenAPI" in agents
    assert "UuidIdValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "party_id" in docs
    assert "POS customer" in docs


@pytest.mark.asyncio
async def test_pos_sale_party_id_api_blank_invalid_422(client, seeded):
    ac, seed = client
    headers = await auth_headers(ac, email="admin@alpha.example.com", tenant_slug="alpha")
    item = {"product_id": seed["p1"].id, "quantity": 1}

    # Ensure an open session exists (party_id 422 should fire before session lookup).
