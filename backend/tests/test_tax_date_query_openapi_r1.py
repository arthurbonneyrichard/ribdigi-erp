"""Tax report from_date/to_date Query OpenAPI honesty."""
...
def test_iso_date_query_schema_for_tax():
...
def test_tax_date_ui_and_docs():
    ...
    assert "Tax date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "Tax From/To date" in docs


@pytest.mark.asyncio
async def test_tax_date_query_blank_invalid_422(client):
    ac, _seed = client
