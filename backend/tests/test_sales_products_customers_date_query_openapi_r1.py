"""Sales products/customers from_date/to_date Query OpenAPI honesty (BR-14.1)."""
...
def test_iso_date_query_schema_for_sales_products_customers():
...
def test_sales_products_customers_date_ui_and_docs():
    ...
    assert "/reports/sales/products" in reports or "sales/products" in reports
    assert "/reports/sales/customers" in reports or "sales/customers" in reports
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "Sales products/customers date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "sales/products" in docs
    assert "sales/customers" in docs


@pytest.mark.asyncio
async def test_sales_products_customers_date_query_blank_invalid_422(client):
    ac, _seed = client
