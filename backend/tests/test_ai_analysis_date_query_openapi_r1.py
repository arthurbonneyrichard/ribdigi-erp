"""AI sales/expenses analysis from_date/to_date Query OpenAPI honesty (BR-21.5 / BR-21.6)."""
...
PATHS = (
    ...
    "/api/v1/ai/expenses/analysis",
)


def test_iso_date_query_schema_for_ai_analysis():
...
def test_ai_analysis_date_ui_and_docs():
    ...
    assert "/ai/sales/analysis" in ai
    assert "/ai/expenses/analysis" in ai
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "AI sales/expenses analysis date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "/ai/sales/analysis" in docs
    assert "/ai/expenses/analysis" in docs


@pytest.mark.asyncio
async def test_ai_analysis_date_query_blank_invalid_422(client):
    ac, _seed = client
