"""RecurringExpenseCreate.category_id ∈ UuidIdValue OpenAPI honesty (BR-9.5)."""
...
def test_recurring_expense_category_id_schema():
    ...
    nullish = RecurringExpenseCreate.model_validate({"amount": 10, "category_id": None})
    assert nullish.category_id is None
    for bad in ("", " ", "!!!", "http://evil", "not-a-uuid", "exp_cat_001", "a b"):
        with pytest.raises(ValidationError):
            RecurringExpenseCreate.model_validate({"amount": 10, "category_id": bad})
...
def test_recurring_expense_category_id_ui_and_docs():
    ...
    assert "Recurring expense category" in docs


@pytest.mark.asyncio
async def test_recurring_expense_category_id_api_blank_invalid_422(client, seeded):
    ac, seed = client
    ...
    for bad in ("", "!!!", "http://evil", "not-a-uuid", "exp_cat_001"):
        resp = await ac.post(
            ...
            headers=headers,
            json={"amount": 12.5, "category_id": bad, "description": "Tip275 bad cat"},
        )
    ...
    ok = await ac.post(
        ...
        json={
            ...
            "description": "Tip275 with category",
            "frequency": "monthly",
        },
    )
    ...
    missing = await ac.post(
        ...
        json={
            ...
            "description": "Tip275 missing category",
        },
    )
