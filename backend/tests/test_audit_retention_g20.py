"""Stage 1 G20 — audit retention policy + cold archive (BR-17.2 / ADR-007)."""
...
@pytest.mark.asyncio
async def test_cold_archive_copies_aged_rows_and_marks_archived(client, db_session, tmp_path, monkeypatch):
    ac, seed = client
    ...
    # Second run should not re-archive the same row
    again = await ac.post(
        "/api/v1/audit-logs/archive-cold?older_than_days=30",
        headers=headers,
    )
