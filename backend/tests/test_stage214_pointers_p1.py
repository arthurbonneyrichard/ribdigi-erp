"""Stage 214 P1 — support runbook RG pointers packaging."""
...
REGISTER = ROOT / "ops" / "mvp" / "support-runbook-rg-pointers.json"


def test_support_runbook_rg_pointers_register_p1():
    data = json.loads(REGISTER.read_text(encoding="utf-8"))
    assert data["stage"] == 214 and data["pack"] == "P1"
    ...
    assert any(
        s["id"] == "sp-sla-remaining" and s["status"] == "remaining"
        for s in data["steps"]
    )
...
def test_support_runbook_rg_pointers_doc_p1():
    doc = (ROOT / "docs/SUPPORT_RUNBOOK_RG_POINTERS_MVP.md").read_text(encoding="utf-8")
