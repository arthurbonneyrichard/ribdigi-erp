"""Stage 152 I1 — platform industries catalog CSV export."""
...
async def _platform_headers(ac, db_engine, email="staff-152i@ribdigi.example.com"):
    ...
    async with session_factory() as db:
        ...
        user = m.User(
            ...
            email=email,
            full_name="Staff 152 Industries",
...
@pytest.mark.asyncio
async def test_platform_industries_export_csv(client, db_engine):
    ac, _seed = client
    headers = await _platform_headers(ac, db_engine)
    exported = await ac.get("/api/v1/platform/industries/export", headers=headers)
    assert exported.status_code == 200, exported.text
    assert "text/csv" in exported.headers.get("content-type", "")
    text = exported.text
    header = text.splitlines()[0]
    assert "row_type" in header and "code" in header and "label" in header
    assert "summary" in text and "catalog" in text
    assert "retail" in text


def test_platform_industries_export_ui_i1():
    page = (ROOT / "frontend/app/platform/tenants/page.tsx").read_text(encoding="utf-8")
    assert "Stage 152" in page
    assert "/platform/industries/export" in page
    assert "Export industries CSV" in page
