"""Cash-flow from_date/to_date Query OpenAPI honesty (BR-14)."""
...
def test_iso_date_query_schema_for_cash_flow():
...
def test_cash_flow_date_ui_and_docs():
    ...
    assert "cashflow" in reports
    assert "/reports/cash-flow" in reports
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "Cash-flow date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "cash-flow" in docs
    assert "IsoDateQueryValue" in docs
    assert "Report From/To date" in docs


@pytest.mark.asyncio
async def test_cash_flow_date_query_blank_invalid_422(client):
    ac, _seed = client
    ...
    assert "inflows" in body or "net" in body or "operating" in body
