"""SalesOrderCreate.customer_id ∈ UuidIdValue OpenAPI honesty (BR-7.3)."""
...
def test_so_customer_id_schema():
    ...
    for bad in ("", "!!!", "http://evil", "not-a-uuid", "cust_001"):
        with pytest.raises(ValidationError):
            SalesOrderCreate.model_validate({"customer_id": bad, "items": _ITEMS})
    with pytest.raises(ValidationError):
        SalesOrderCreate.model_validate({"items": _ITEMS})


def test_so_customer_id_ui_and_docs():
    ...
    assert "Sales order customer_id OpenAPI" in agents
...
@pytest.mark.asyncio
async def test_so_customer_id_api_blank_invalid_422(client, seeded):
    ac, seed = client
