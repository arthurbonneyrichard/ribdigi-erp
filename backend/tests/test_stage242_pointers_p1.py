"""Stage 242 P1 — customer training cert pack RG pointers packaging."""
...
REGISTER = ROOT / "ops" / "mvp" / "customer-training-cert-pack-rg-pointers.json"


def test_customer_training_cert_pack_rg_pointers_register_p1():
    data = json.loads(REGISTER.read_text(encoding="utf-8"))
    assert data["stage"] == 242 and data["pack"] == "P1"
    ...
    for topic in (
        ...
        "live_training_pack_remaining_gate_stage241",
    ...
    assert any(
        s["id"] == "ctcprp-training-remaining" and s["status"] == "remaining"
        for s in data["steps"]
    )
...
def test_customer_training_cert_pack_rg_pointers_doc_p1():
    doc = (ROOT / "docs/CUSTOMER_TRAINING_CERT_PACK_RG_POINTERS_MVP.md").read_text(
        encoding="utf-8"
    )
