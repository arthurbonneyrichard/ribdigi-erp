"""LineItem / SalesInvoiceItemCreate.unit_id ∈ UuidIdValue OpenAPI honesty (BR-7.4 / BR-8.1)."""
...
_CUSTOMER = "bbbbbbbb-cccc-dddd-eeee-ffffffffffff"


def test_line_item_unit_id_schema():
    ...
    si = SalesInvoiceItemCreate.model_validate(
        {"product_id": _PRODUCT, "quantity": 1, "unit_id": _VALID}
    )
    assert si.unit_id == _VALID.lower()
    with pytest.raises(ValidationError):
        SalesInvoiceItemCreate.model_validate(
            {"product_id": _PRODUCT, "quantity": 1, "unit_id": "!!!"}
        )
...
def test_line_item_unit_id_ui_and_docs():
    ...
    assert "Line item unit_id OpenAPI" in agents
    assert "UuidIdValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "Sales unit" in docs
    assert "unit_id" in docs


@pytest.mark.asyncio
async def test_line_item_unit_id_api_blank_invalid_422(client, seeded):
    ac, seed = client
    ...
    customer = await ac.post(
        "/api/v1/customers",
        headers=headers,
        json={"name": f"TIP367 Customer {suffix}"},
    )
