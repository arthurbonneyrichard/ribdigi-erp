"""GET /inventory|/reports/inventory/movements Query store_id ∈ UuidIdValue (BR-5.3)."""
...
def test_movement_store_id_query_schema():
...
def test_movement_store_id_query_ui_and_docs():
    ...
    assert "Movement store_id Query OpenAPI" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "store_id" in docs
    assert "UuidIdValue" in docs


@pytest.mark.asyncio
async def test_movement_store_id_query_blank_invalid_422(client):
    ac, _seed = client
