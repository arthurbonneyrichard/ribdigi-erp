"""Sales returns/salesperson from_date/to_date Query OpenAPI honesty (BR-14.1)."""
...
def test_iso_date_query_schema_for_sales_returns_salesperson():
...
def test_sales_returns_salesperson_date_ui_and_docs():
    ...
    assert "sales/returns" in reports
    assert "sales/salesperson" in reports
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "Sales returns/salesperson date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "sales/returns" in docs
    assert "sales/salesperson" in docs


@pytest.mark.asyncio
async def test_sales_returns_salesperson_date_query_blank_invalid_422(client):
    ac, _seed = client
