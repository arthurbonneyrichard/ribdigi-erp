"""Purchases pending-orders/returns from_date/to_date Query OpenAPI honesty (BR-14.3)."""
...
def test_iso_date_query_schema_for_purchases_pending_returns():
...
def test_purchases_pending_returns_date_ui_and_docs():
    ...
    assert "purchases/pending-orders" in reports
    assert "purchases/returns" in reports
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "Purchases pending/returns date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "purchases/pending-orders" in docs
    assert "purchases/returns" in docs


@pytest.mark.asyncio
async def test_purchases_pending_returns_date_query_blank_invalid_422(client):
    ac, _seed = client
