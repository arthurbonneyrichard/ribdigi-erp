"""OpenAPI honesty tips #693–#718: optional narrative defense + money_json + residual aria."""
...
def test_honesty_batch3_docs_and_agents():
    ...
    for title in (
        ...
        "PO amend unit price/discount aria OpenAPI",
        "Purchase invoice FX rate aria OpenAPI",
        "Purchase invoice tax rate percent aria OpenAPI",
    ...
    assert "sales order" in standards.lower()
    assert "bank statement" in standards.lower()
...
def test_optional_honest_narrative():
    ...
    assert optional_honest_narrative("Shift OK", label="notes") == "Shift OK"
    with pytest.raises(HTTPException) as exc:
        optional_honest_narrative("!!!", label="notes")
    assert exc.value.status_code == 400
    assert "plain narrative" in exc.value.detail
    with pytest.raises(HTTPException) as url:
        optional_honest_narrative("http://evil", label="notes")
    assert url.value.status_code == 400
