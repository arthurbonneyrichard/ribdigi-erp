"""Inventory transfers/stock-counts from_date/to_date Query OpenAPI honesty (BR-13.2 / BR-5.2)."""
...
PATHS = (
    ...
    "/api/v1/reports/inventory/stock-counts",
)


def test_iso_date_query_schema_for_inventory_transfers_stock_counts():
...
def test_inventory_transfers_stock_counts_date_ui_and_docs():
    ...
    assert "inventory/transfers" in reports
    assert "inventory/stock-counts" in reports
    agents = (ROOT / "AGENTS.md").read_text(encoding="utf-8")
    assert "Inventory transfers/stock-counts date Query OpenAPI" in agents
    assert "IsoDateQueryValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "IsoDateQueryValue" in docs
    assert "inventory/transfers" in docs
    assert "inventory/stock-counts" in docs


@pytest.mark.asyncio
async def test_inventory_transfers_stock_counts_date_query_blank_invalid_422(client):
    ac, _seed = client
