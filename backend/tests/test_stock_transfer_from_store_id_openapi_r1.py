"""StockTransferCreate.from_store_id ∈ UuidIdValue OpenAPI honesty (BR-5.2 / BR-13.2)."""
...
def test_stock_transfer_from_store_id_schema():
    assert _uuid.validate_python(f"  {_VALID_FROM}  ") == _VALID_FROM.lower()
    ok = StockTransferCreate.model_validate(
        {
            "from_store_id": f"  {_VALID_FROM}  ",
            "to_store_id": _VALID_TO,
            "items": _ITEMS,
        }
    )
    assert ok.from_store_id == _VALID_FROM.lower()
    ...
    assert omit_ok.from_store_id is None
    ...
    assert nullish.from_store_id is None
...
def test_stock_transfer_from_store_id_ui_and_docs():
    ...
    assert "Stock transfer from_store_id OpenAPI" in agents
    assert "UuidIdValue" in agents
    docs = (ROOT / "docs/API_DOCUMENTATION.md").read_text(encoding="utf-8")
    assert "Stock transfer from store" in docs
    assert "POST /stores/transfers" in docs


@pytest.mark.asyncio
async def test_stock_transfer_from_store_id_api_blank_invalid_422(client, db_session, seeded):
    ac, seed = client
    ...
    to_store = await create_store(
        ...
        name=f"Tip344 To {suffix}",
        code=f"T344{suffix[:4]}".upper(),
    )
    ...
    missing = await ac.post(
        ...
        headers=headers,
        json={
            "from_store_id": f"  {str(uuid4()).upper()}  ",
            "to_store_id": to_store.id,
            "submit": False,
            "items": [{"product_id": product_id, "quantity": 1}],
        },
