...
async def stock_import(
    ...
    dashboard_scope_svc.assert_company_level_stock_import_denied(
        managed,
        message="Store managers cannot bulk-import stock across company warehouses.",
    )
    raw = await file.read()
    if not raw:
        raise HTTPException(status_code=400, detail="Empty upload")
...
async def product_image_get(
    ...
    managed = await dashboard_scope_svc.managed_store_ids(db, claims)
    dashboard_scope_svc.assert_company_level_product_primary_image_read_denied(managed)
    product = (
        await db.execute(
            select(m.Product).where(
                m.Product.id == product_id,
...
async def list_purchase_requests(
    ...
    rows = (await db.execute(stmt)).scalars().all()
    out = [await purchasing_svc.serialize_pr(db, pr) for pr in rows]
...
async def create_purchase_request(
    ...
    await db.commit()
    payload_out = await purchasing_svc.serialize_pr(db, pr)
    if dashboard_scope_svc.omit_approval_matrix_roles(managed_wh):
        payload_out = dashboard_scope_svc.redact_approval_matrix_roles(payload_out)
    return env(payload_out, "Purchase request created")
...
async def reject_purchase_request(
    ...
    if dashboard_scope_svc.omit_approval_matrix_roles(managed_wh):
        payload_out = dashboard_scope_svc.redact_approval_matrix_roles(payload_out)
    return env(payload_out, "Purchase request rejected")
...
async def cancel_purchase_request(
    ...
    if dashboard_scope_svc.omit_approval_matrix_roles(managed_wh):
        payload_out = dashboard_scope_svc.redact_approval_matrix_roles(payload_out)
    return env(payload_out, "Purchase request cancelled")
...
async def expenses_recurring_export(
    ...
    text = await liquid_recurring_export_svc.export_recurring_expenses_csv(
        ...
        company_id=claims.get("company_id"),
        store_ids=managed,
        omit_department_id=dashboard_scope_svc.omit_expense_department_assignment(managed),
    )
    return Response(
        content=text,
        media_type="text/csv",
...
async def expense_ocr_suggest(
    ...
    if dashboard_scope_svc.omit_approval_matrix_roles(managed):
        result = dashboard_scope_svc.redact_approval_matrix_roles(result)
    return env(result, "OCR suggestions ready — review before applying")
...
async def bank_feed_settings(
    ...
    managed = await dashboard_scope_svc.managed_store_ids(db, claims)
    dashboard_scope_svc.assert_company_level_bank_feed_settings_read_denied(managed)
...
async def get_fiscal_period(
    ...
    managed = await dashboard_scope_svc.managed_store_ids(db, claims)
    dashboard_scope_svc.assert_company_level_fiscal_period_read_denied(managed)
    tenant = (
...
async def report_sales_customers(
    ...
    managed = await dashboard_scope_svc.managed_store_ids(db, claims)
    data = await reports_svc.sales_by_customer(
        db,
        claims["tenant_id"],
        from_date=reports_svc.parse_date(from_date),
        to_date=reports_svc.parse_date(to_date, end_of_day=True),
    ...
    data = dashboard_scope_svc.apply_sales_customers_manager_redacts(data, managed)
    return env(data)
...
async def credit_settings(
    ...
    managed = await dashboard_scope_svc.managed_store_ids(db, claims)
    dashboard_scope_svc.assert_company_level_credit_settings_read_denied(managed)
    tenant = await tenants_svc.get_tenant(db, claims["tenant_id"])
...
async def update_store(
    ...
    changing_manager = payload.manager_id is not None or bool(
        getattr(payload, "clear_manager", False)
    )
    dashboard_scope_svc.assert_store_manager_assignment_write_denied(
        managed,
...
async def inventory_settings(
    ...
    managed = await dashboard_scope_svc.managed_store_ids(db, claims)
    dashboard_scope_svc.assert_company_level_inventory_settings_read_denied(managed)
    tenant = await tenants_svc.get_tenant(db, claims["tenant_id"])
    return env(
        {
...
async def audit_logs_export(
    ...
    if (
        ...
        or dashboard_scope_svc.omit_audit_expense_threshold_details(managed)
        or dashboard_scope_svc.omit_audit_cle_invoice_total_details(managed)
    ):
...
async def backup_verify(
    ...
    await db.commit()
    msg = "Backup verify passed" if (report.get("proof") or {}).get("ok") else "Backup verify found mismatches"
...
async def ai_expenses_analysis(
    ...
    if dashboard_scope_svc.omit_approval_matrix_roles(managed_stores):
        data = dashboard_scope_svc.redact_approval_matrix_roles(data)
    return env(data)
