...
def assert_bank_connection_lifecycle_write_denied(
    ...
    *,
    ...
    Create/delete are separately denied; soft activate/deactivate stays
    admin-only. Display name / feed patches on managed connections remain.
    """
    if not changing_active:
        return
    assert_company_level_write_denied(managed_ids, message=message)
...
def assert_bank_connection_credentials_write_denied(
    ...
    *,
    ...
    ``display_name``, auto-sync flags, and lookback on managed connections
    remain allowed. ``is_active`` is gated by
