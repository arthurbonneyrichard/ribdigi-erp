...
| RBAC / store scope | **PARTIAL** — store_manager scoped via `manager_id` across ops surfaces + inventory/sales/purchasing reports + transfer history + stock-transfer writes + expenses summary/budgets/export + expense write asserts + AI store+WH scoped + credit AR/AP aging (+ export/dashboard/BI) store+WH scoped + credit statements/outstanding/payment registers + payment write asserts store+WH scoped + accounting P&L/TB/cash-flow/balance-sheet (+ exports; dashboard MTD P&L) store scoped + **tax report/filing (+ exports) store+WH scoped**; ADR-005 still open |
...
- **Store RBAC ops hardening:** store + warehouse helpers through POS/sales/expenses/stores/transfers, warehouses/inventory ops, purchasing PR/PO/GRN/returns/invoices, sales orders, POS sessions, low-stock/expiry, inventory + sales reports, **purchasing reports**, **transfer history**, **inventory stock-transfer write asserts**, **expenses summary/budgets/export** (approved spend + pending variance scoped; null-store fail-closed), **expense + recurring create / expense patch store asserts**, **AI inventory** low-stock / demand-forecast / dead-stock / predictions (+ exports; chat stockout intent) scoped to managed WH stock + store sales, **AI insights + sales/expenses/purchases/cross-domain analysis** (+ exports) similarly scoped, **AI customer insights/assist** (+ export; chat customer count) from managed-store sales only, **AI chat** top-product / sales-month / expenses / classic low-stock helpers (store+WH scoped; empty managed → empty/zero), **AI documents/analyze** (+ export) party/product matches store+WH scoped, **credit AR/AP aging** (+ export; dashboard/BI) store+WH scoped, **credit statements/outstanding/payment registers + payment write asserts** store+WH scoped, **accounting P&L/TB/cash-flow/balance-sheet** (+ exports; dashboard MTD P&L) store scoped, and **tax report/filing (+ exports)** store+WH scoped (null-store/null-WH fail-closed). Expense auto-journals now copy `company_id`. Alembic `20260823_0109`. Tests: `test_store_scope_ops_hardening.py` (+ AI + credit + accounting + tax suites). Does not claim store-scoped RBAC Complete or ADR-005. Audit company-read may still be company-wide.
