...
| RBAC / store scope | **PARTIAL** — store_manager scoped via `manager_id` across ops surfaces + inventory/sales/purchasing reports + transfer history + stock-transfer writes + expenses summary/budgets/export + expense/recurring create + expense patch store asserts + AI inventory/insights/cross-domain/customers/chat/security/documents store+WH scoped + **credit AR/AP aging (+ export, dashboard credit slice, BI credit, reports export) store+WH scoped**; ADR-005 still open |
