...
| RBAC / store scope | **PARTIAL** — store_manager scoped via `manager_id` across ops surfaces + inventory/sales/purchasing reports + transfer history + stock-transfer writes + expenses summary/budgets/export + expense/recurring create + expense patch store asserts + AI inventory low-stock/forecast/dead-stock/predictions (+ exports) WH+store scoped + AI insights / sales / expenses / purchases / cross-domain analysis (+ exports) store+WH scoped + AI customer insights/assist (+ export; chat customer count) store scoped + AI chat top-product / sales-month / expenses / low-stock helpers store+WH scoped + AI security alerts (+ export) self + details.store_id/warehouse_id fail-closed + **AI documents/analyze (+ export) party/product matches store+WH scoped**; ADR-005 still open |
