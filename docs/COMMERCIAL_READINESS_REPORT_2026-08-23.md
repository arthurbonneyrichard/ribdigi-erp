...
| RBAC / store scope | **PARTIAL** — store_manager scoped via `manager_id` across ops surfaces + inventory/sales/purchasing reports + transfer history + stock-transfer writes + expenses summary/budgets/export + expense write asserts + AI store+WH scoped + credit AR/AP aging (+ export/dashboard/BI) store+WH scoped + **credit statements/outstanding/payment registers + payment write asserts store+WH scoped**; ADR-005 still open |
