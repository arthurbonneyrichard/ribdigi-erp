...
| RBAC / store scope | **PARTIAL** — store_manager scoped via `manager_id`→`Warehouse.store_id` across POS/sales/expenses/stores/transfers/inventory ops/purchasing PR/PO/GRN/returns + purchase invoices (direct `warehouse_id` or linked PO/GRN WH) + sales orders + POS sessions + low-stock / expiry list+export+reports + **inventory balance/valuation/movements reports + `/reports/summary` low-stock + inventory export types**; ADR-005 still open |
...
9. **Store RBAC** — **PARTIAL** (2026-08-23): `store_manager` constrained on POS/sales/expenses/stores/transfers, warehouse inventory ops, purchasing PR/PO/GRN/returns, purchase invoices (direct `warehouse_id` or linked GRN/PO WH; unlinked null-WH still fail-closed), **sales orders**, **POS sessions**, **low-stock / expiring batches**, and **inventory balance/valuation/movements reports** (+ `/reports/summary` low-stock + matching `/reports/export` inventory types; product.stock_qty fallback omitted). ADR-005 membership still open.
...
- **Store RBAC ops hardening:** store + warehouse helpers through POS/sales/expenses/stores/transfers, warehouses/inventory ops, purchasing PR/PO/GRN/returns, purchase invoices (direct `warehouse_id` preferred, else PO/GRN join), **sales orders**, **POS sessions**, **low-stock / expiry**, and **inventory balance/valuation/movements reports** (+ summary low-stock + export). Alembic `20260823_0109`. Tests: `test_store_scope_ops_hardening.py`. Does not claim store-scoped RBAC Complete or ADR-005.
