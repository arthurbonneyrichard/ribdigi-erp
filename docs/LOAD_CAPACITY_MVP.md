# Load Capacity MVP (Stage 26 C1 + Stage 28 C1 pack)

**Status:** Documented — Stage 26 C1 CI capacity evidence; Stage 28 C1 operator ~1000-VU cert packaging  
**Product:** RIBDIGI BUSINESS ERP — Commercial MVP  
**Related:** Stage 5 L1 (`test_loadtest_baseline_l1.py`), Stage 18 T1 (`test_loadtest_evidence_t1.py`), Stage 26 C1 (`test_load_capacity_c1.py`), Stage 28 C1 (`test_load_cert_pack_c1.py`)  
...
## Operator staging 1000-VU checklist (Stage 28 C1 packaging)

Authoritative pack: [LOAD_CERT_PACK_MVP.md](LOAD_CERT_PACK_MVP.md) · `ops/loadtest/1000vu-cert-checklist.json` · `ops/loadtest/operator_1000vu_run.example.json`.
...
5. Fill the operator run JSON (schema example) and attach to the launch change log.

Optional Locust: `backend/loadtest/locustfile.py` (see `LOAD_TEST_BASELINE.md`). Live execution remains **Remaining** until an ops change-log entry exists — packaging evidence keeps `operator_1000vu_executed: false`.

## Explicitly deferred

- Certified ~1000-VU staging capacity certificate in CI (Stage 28 C1 packs checklist/schema only)
- PgBouncer / Redis app-cache capacity labs
- Vendor ZAP-in-CI Top 10 under load

## Sign-off

Stage 26 C1 is met when smoke + CI capacity profiles pass, the evidence artifact exists, and PRODUCTION_READINESS load gate is Complete (MVP) with Remaining limited to operator staging ~1000-VU **execution**. Stage 28 C1 is met when `docs/LOAD_CERT_PACK_MVP.md` + checklist + run schema + `test_load_cert_pack_c1.py` pass without forging a VU certificate.
