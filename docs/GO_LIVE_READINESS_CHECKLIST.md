...
store-scoped RBAC Completes. ADR-005 membership is **Complete** (flag default OFF). First-class `export`/`view_cost` RBAC actions are **Complete** for the engine slice (admin/settings/catalog module gates closed; intentional role-gated admin dumps + caller-scoped session/passkey exports retained; not overall RBAC Complete). Temp membership `expires_at` is **PARTIAL** (elevation/break-glass MISSING). Cloud-agent Chrome could not finish
