...
Create body `PlatformStaffCreate` (`extra=forbid`): `email` ∈ `EmailStr` (blank/`not-an-email`/`abc` → **422**; was free `str` with `min_length=3`); create/patch `role` uses the same platform-role schema `Literal` as grant (strip/lower; create omit → `platform_support`; blank/invalid → **422**; no silent support from `""` via former `role or "platform_support"`). Platform Staff **Platform staff email** + **Create platform staff**.  
