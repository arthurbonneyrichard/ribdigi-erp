...
**Create:** `POST /expenses/recurring` — `payment_method` same expense `Literal` (omit → `bank_transfer`; blank/invalid → **422**). Optional `description` ∈ `ExpenseDescriptionValue` (omit/`null` → empty; blank/`!!!`/`http://…` → **422**). Expenses **Recurring description** input.  
**Update:** `PATCH /expenses/recurring/{id}` — activate/deactivate (`is_active`) and/or edit template fields: `amount`, `payee` (+ `clear_payee`), `description` ∈ `ExpenseDescriptionValue` (omit/`null` → no change; blank/`!!!`/`http://…` → **422**), `payment_method` (same `Literal`, omit = no change; blank/invalid → **422**), `frequency`, `category_id` / `category`, org dims (`branch_id` / `department_id` + clear flags). Expenses UI **Edit schedule**. Existing generated expenses are unchanged; next Generate uses the updated template.  
