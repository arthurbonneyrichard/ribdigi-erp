...
**Create:** `POST /purchasing/requests` — optional `required_date` ∈ `IsoDateQueryValue` (strip; `YYYY-MM-DD` or ISO datetime); omit/`null` → no needed-by date; blank/`not-a-date`/`01/02/2024` → **422** (was free `datetime`; OpenAPI date-time; padded dates inconsistent). API `reports.parse_date` remains defense-in-depth. Optional `notes` ∈ `PurchaseRequestNotesValue` (strip; 1–500; ≥1 letter/digit; no `://`/`@`); omit/`null` → no notes; blank/`!!!`/`http://…` → **422** (was free `str`; blank/garbage could persist on PREQ). Purchasing **Purchase request required date** + **Purchase request notes** inputs (`aria-label`s); create sends `null` when blank.  
