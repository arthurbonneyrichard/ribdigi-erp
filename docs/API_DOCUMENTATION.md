...
**Update:** `PATCH /catalog/categories/{category_id}` — `code` ∈ `CategoryCodeValue` (omit/`null` → no change; blank/`!!!`/`http://…` → **422**); `name` ∈ `CategoryNameValue` (omit/`null` → no change; blank/`!!!`/`http://…` → **422**); optional `parent_id` ∈ `UuidIdValue` (omit → no change; `null` → root; blank/`!!!`/`http://…`/non-UUID → **422** — was free `str`; garbage could reach parent lookup; existence / cycle checks remain tenant-scoped **404**/400). Inventory Catalog **Edit category parent** select; reparent sends trim or `null` when blank. Soft-reactivate via `is_active: true` (Inventory Catalog **Activate**)  
