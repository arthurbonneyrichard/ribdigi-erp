...
- Currency / money fields in **JSON responses** are likewise JSON **numbers**. Response serializers use `app.honesty.money_json` (ORM `Numeric`/`Decimal` → finite IEEE-754 float; NaN/Inf rejected — not decimal strings) on sales invoice, quotation, sales order, sales return, purchase order, purchase invoice, purchase return, GRN qty, expense/recurring, cheque, journal, cash account/transfer, bank statement/line, POS payment/drawer, FX rate, purchase-request qty, stock-count qty, stock-transfer qty, product/variant/batch, tax rate, expense-category budget, customer-group discount, unit conversion ratio, tenant expense/early-pay thresholds, and warehouse capacity pilots. Other serializers may still use bare `float(...)`; CSV product export may still render prices as text columns.
