...
**Honesty:** Live fan-out today includes `webhook.test`, `sale.created`, `sale.paid`, `customer.created`, `purchase.order.created`, `purchase.grn.received`, `expense.approved`, `stock.low`, and `stock.in` (skips GRN — use `purchase.grn.received`). Still reserved: `user.login`, `tenant.suspended`.
