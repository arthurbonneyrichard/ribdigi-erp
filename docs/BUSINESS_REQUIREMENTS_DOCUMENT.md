...
- **Acceptance Criteria:**
  - [x] Support statuses: Trial, Active, Suspended
    - Complete (MVP): also `grace`; platform list/filter + suspend/activate with required **Suspend reason** (`POST /tenants/{ref}/suspend` `{ reason }` → `suspended_reason`; Platform console input — no `window.prompt`)
...
- **Acceptance Criteria:**
  ...
  - [x] **Stock Count:** Physical count reconciliation with system stock; variance report generation (`GET|POST /inventory/stock-counts` + complete posts variances; draft **Cancel** via `POST /inventory/stock-counts/{id}/cancel` + Inventory Cancel UI; `GET /reports/inventory/stock-counts` variance report + export `inventory_stock_counts`; Reports Inventory panel; `count_number` via tenant series `GET|PATCH /inventory/settings` → `stock_count_numbering`; default `SC`; Inventory Document numbering UI)
...
- **Acceptance Criteria:**
  ...
  - [x] Status tracking: Draft, Pending, Approved, Rejected, Converted to PO (`rejection_reason` on reject via `POST /purchasing/requests/{id}/reject`; Purchasing Reject reason field — no hardcoded UI reason)
...
- **Acceptance Criteria:**
  ...
  - [x] Cheque management (issue, deposit, bounce tracking) (`POST /accounting/cheques/{id}/bounce|cancel?reason=`; Accounting Cheques Bounce/Cancel reason field → notes + journal description)
...
- **Acceptance Criteria:**
  ...
  - [x] Block sales that exceed credit limit (with override permission) (`override_credit_limit` + required UI `override_reason` on Sales Post / POS credit; no canned `window.prompt` default; audit `credit_limit_override`)
