...
- **Acceptance Criteria:**
  ...
  - [x] **Stock Transfer:** Move stock between warehouses with transfer note, approval workflow (`POST /inventory/stock-transfers` accepts `from_warehouse_id`/`to_warehouse_id`; same-store = 1-step approval, inter-store = dual; ship/receive; **Reject reason** via `POST .../reject` `{ reason }` → `rejection_reason` + status `cancelled`; Inventory Transfers tab; aliases under `/inventory/stock-transfers*`; `transfer_number` via tenant series `GET|PATCH /inventory/settings` → `stock_transfer_numbering`; default `TR`)
